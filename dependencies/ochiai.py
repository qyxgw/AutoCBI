...
for gcc_revision in gcc_revisions:
    ...
    ochiai={}
    for key in cfs:
        ochiai[key]=float(cfs[key]/((cfs[key]+cps[key])*cfs[key])**0.5)
    ...
    #     else:
    #         gcccount[key]=0
    sorted_dict = dict(sorted(ochiai.items(), key=lambda x: x[1], reverse=True))
    ...
    f=open('/data/AutoCBI/ochiai/rankFile_'+bugid+'.txt','a')
    loop=20
