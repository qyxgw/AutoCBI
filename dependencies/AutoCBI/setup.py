...
for i in range(len(revlines)):
    ...
    if bugId not in gccbugsname:
        ...
        if os.path.exists(infodir + "/" + bugId):
            os.system("rm -rf {}".format(infodir + "/" + bugId))
...
# from AutoCBI.llvm import installSVN, collect
...
for i in range(len(revlines)):
    ...
    whether_installs.append(revlines[i].strip().split(',')[5])
    wrongs.append(revlines[i].strip().split(',')[3])
from AutoCBI.gcc import installSVN, collect
...
maindir = cfg.get('gcc-locations', 'maindir')
RecBidir = maindir + '/../'
