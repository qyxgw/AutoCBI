import os
from dependencies.AutoCBI.AutoCBI.util import exccmd
...
def collect(compilersdir, infodir, bugIds, revisions, wrongoptions):
    ...
    for i in range(len(bugIds)):
        ...
        wrongoption = wrongoption.replace("+", " ")
        bugId = bugIds[i]
        ...
        exccmd(gccdir + '/clang -fprofile-arcs -ftest-coverage ' + wrongoption + ' ' + testname + '.c')  # change per bug
        if os.path.exists('gcdalist'):

            exccmd('rm gcdalist')
        exccmd('find ' + covdir + ' -name \"*.gcda\" > gcdalist')
        print('find ' + covdir + ' -name \"*.gcda\" > gcdalist')
        ...
        for i in range(len(lines)):
            ...
            if os.path.exists('gcovfile'):
                exccmd('rm gcovfile')
            print('gcov-7 -f ' + gcdafile + ' > gcovfile')
            exccmd('gcov-7 -f ' + gcdafile + ' > gcovfile')
