...
#sys.path.append('/data01/qiyixian/DiWi-master')
...
compiler=sys.argv[2]
compilationOptionsRight=sys.argv[3].replace('+',' ')
compilationOptionsWrong=sys.argv[4].replace('+',' ')
checkpass=sys.argv[5]
roundcount=sys.argv[6]
prefixpath='/data/compilers/gcc/'+compiler+'/'+compiler
mcmcprefix='/data01/qiyixian/DiWi/gcc/result/MCMC_new_r/'+roundcount+'/'+revisionnumber
locationpath='/data01/qiyixian/DiWi-master/benchmark/gccbugs/'+revisionnumber
...
srcdir=prefixpath
resdir=mcmcprefix
exccmd('cp /data01/qiyixian/DiWi-master/mutation/* .')
...
# print ('start')
failcovpath='/data01/qiyixian/DiWi/gcc/gccInfo'
if os.path.exists(failcovpath+'/'+revisionnumber+'/failcov/stmt_info.txt'):
    tarpath=failcovpath+'/'+revisionnumber+'/failcov/stmt_info.txt'
elif os.path.exists(failcovpath+'/'+revisionnumber+'/fail/stmt_info.txt'):
    tarpath=failcovpath+'/'+revisionnumber+'/fail/stmt_info.txt'
else:
    print("Error!!")
    sys.exit(1)
...
while(cnt<500):
    ...
    if len(difflines)==0:
        ...
        if checkpass=='checkIsPass_wrongcodeOneline':
            flagIsPass=failmessage.checkIsPass_wrongcodeOneline(compiler,compilationOptionsRight,compilationOptionsWrong) # 1:pass; 2:still fail
        elif checkpass=='checkIsPass_zeroandsegmentoneline':
            flagIsPass=failmessage.checkIsPass_zeroandsegmentoneline(compiler,compilationOptionsRight,compilationOptionsWrong) # 1:pass; 2:still fail
        elif checkpass=='checkIsPass_zeroandonenumber':
            flagIsPass=failmessage.checkIsPass_zeroandonenumber(compiler,compilationOptionsRight,compilationOptionsWrong) # 1:pass; 2:still fail
        elif checkpass=='checkIsPass_multilineswrongcode':
            flagIsPass=failmessage.checkIsPass_multilineswrongcode(compiler,compilationOptionsRight,compilationOptionsWrong) # 1:pass; 2:still fail
        elif checkpass=='checkIsPass_onenumberandzero':
            flagIsPass=failmessage.checkIsPass_onenumberandzero(compiler,compilationOptionsRight,compilationOptionsWrong) # 1:pass; 2:still fail
        elif checkpass=='checkIsPass_onenumberandsegmentoneline':
            flagIsPass=failmessage.checkIsPass_onenumberandsegmentoneline(compiler,compilationOptionsRight,compilationOptionsWrong) # 1:pass; 2:still fail
